...
# Backup sormas_db
sudo -u postgres pg_dump -Fc -b sormas_db > "/root/deploy/sormas/backup/sormas_db_"$d".dump"

# Check the exit status of the previous command
if [ $? -eq 0 ]; then
    echo "Backup of sormas_db successful."
else
    echo "Backup of sormas_db failed."
fi

# Backup sormas_audit_db
sudo -u postgres pg_dump -Fc -b sormas_audit_db > "/root/deploy/sormas/backup/sormas_audit_db_"$d".dump"

# Check the exit status of the previous command
if [ $? -eq 0 ]; then
    echo "Backup of sormas_audit_db successful."
else
    echo "Backup of sormas_audit_db failed."
fi
