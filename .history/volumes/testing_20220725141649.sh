...
if [[ $mode = 'replica' ]] || [[ $platform = 'docker' ]]
then
...
echo "***************************Restarting PG CLUSTER*********************************************"
sudo pg_ctlcluster 10 "$this_instance_name" restart

echo "***************************Replica files deleting*********************************************"
# clean directory of db instance
if [ $mode = 'replica' ]
then
    # sudo su
    echo "***************************Creating delete*********************************************"
    source /tmp/mnt/env.sh
    cd "$this_instance_var_path" || exit
...
echo "***************************Giving postgres permission*********************************************"
