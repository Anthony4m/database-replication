...
production_ip=172.20.0.3 #ip a. look for eth or eth0. pick the ip that has a subnet. eg 127.20.0.3/16
...
echo "password" > "$this_instance_etc_path"/userpass
...
#  sudo su
    ...
    # echo "here"
    echo sudo find . -name . -o -prune -exec rm -rf -- {} +
    echo "here"
    echo exit
...
"*******************PGBACKUP******************************"
    sudo pg_basebackup --dbname postgresql://rep_user:password@$other_instance_ip:$other_instance_port/postgres?password=rep_user_123 -D $this_instance_var_path
